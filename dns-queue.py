...
class RealProber(threading.Thread):
    def __init__(self, dns_server, target, dns_timeout, results_collector, error_collector):
        ...
        self.res = res if results_collector is None else results_collector
        self.err = err if error_collector is None else error_collector
    ...
    def run(self):
        ...
        try:
            log.debug(
                f"{self.name}: Resolving {self.target} with nameserver {self.dns_server}"
            )
            ...
            for data in answer:
                out = f'{self.target} | {data}'
                self.res.append(out)
                        # don't log to console, use file.
                        # log.info(out)
        except dns.exception.Timeout as e:
            # we want to know if the DNS server is barfing
            errmsg = f"{self.target}: {e}"
            self.err.append(errmsg)
                # log.warn(errmsg)
        except dns.exception.DNSException as e:
            log.debug(f"Error in thread {self.name} when querying {self.target}: {e}")
...
class MockProber(RealProber):
    ...
    def run(self):
        ...
        log.debug(f"Mock probe {self.name} sleeping for {_sleep_for}...")
        time.sleep(_sleep_for)
        if random.random() > 0.7:
            log.debug("Mock probe {} found a result.")
            res.append(f"{self.target} | 127.0.0.1")
        log.debug(f"Mock prober {self.name} done")
...
class LoggingThread(threading.Thread):
    ...
    def __init__(self, log_event, outfile):
        ...
        if outfile is not None:
            prGreen(f"[+] Saving results to {outfile}...")
...
def random_subdomain():
    ...
    wildcard DNS"""
    for _ in range(RANDOM_SUBDOMAINS):
        yield ''.join(random.choice(ALPHABET) for _ in range(RANDOM_SUBDOMAINS_LENGTH))
...
def subdomain_len(max_subdomain_len):
    import math
    return sum(
        math.factorial(len(ALPHABET)) / math.factorial(len(ALPHABET) - i - 1)
        for i in range(max_subdomain_len)
    )
...
def do_check_wildcard_dns(dom, nsvrs, dns_timeout):
    ...
    try:
        ...
        for _ in range(len(wildcard_checklist)):
    ...
    if len(wildcard_results) == RANDOM_SUBDOMAINS:
        raise SystemExit(
            f"{RANDOM_SUBDOMAINS} random subdomains returned a hit; It is likely this is a wildcard DNS server. Use the -w option to skip this check."
        )
...
def main(dom,
    ...
    # output management
    #
    print(f"[+] Output destination: '{outfile}'")
    if outfile is not None and os.path.exists(outfile):
        if overwrite is False:
            raise SystemExit(
                f"Specified file '{outfile}' exists and overwrite option (-f) not set"
            )
    ...
    if infile is None:
        ...
        prGreen(
            f"[+] Will search for subdomains made of all possible {max_subdomain_len}-characters permutations"
        )

    else:
        if not os.path.exists(infile):
            raise SystemExit(f"{infile} not found")
        sub = subdomain_fromlist(infile)
        total_domains = subdomain_fromlist_len(infile)
        prGreen(f"[+] Will search for subdomains contained in '{infile}'")
    ...
    log.debug(f"Progressbar initialised with {total_domains} max")
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help=f"Maximum length of the subdomain for bruteforcing. Default: {DEFAULT_MAX_SUBDOMAIN_LEN}",
    )
    ...
    parser.add_argument(
        ...
        help=f"How long to wait for a DNS response. Default: {DEFAULT_DNS_TIMEOUT}s",
    )
